...
def plot_matrix(X, Fs=1, Fs_F=1, T_coef=None, F_coef=None, freq_xlabels=5, xlabel='Time (seconds)', ylabel='Frequency (Hz)', title='',
    ...
    if ax is None:
        ...
        ax = [ax]
    if T_coef is None:
        T_coef = np.arange(0, X.shape[1]/ Fs, freq_xlabels)
    if F_coef is None:
...
def compute_tempogram_autocorr(x, Fs, N, H, norm_sum=True, Theta=[]):
